...
class DataProcess(val context: Context) {
    ...
    private val TAG = DataProcess::class.java.simpleName
    ...
    companion object {
        ...
        const val LABEL_NAME = "best.txt"
        /*const val FILE_NAME = "yolo11n.onnx"
        const val LABEL_NAME = "yolo11n.txt"*/
    }
    ...
    fun outputsToNPMSPredictions(outputs: Array<*>): ArrayList<Result> {
        ...
        val minBoxSize = 10 // 최소 너비와 높이 (픽셀 단위)
        ...
        for (i in 0 until cols) {
            ...
            for (j in classes.indices) {
                if (classArray[j] > maxScore) {
                    ...
                    //Log.i(TAG, "detectionClass: $detectionClass, maxScore: $maxScore")
                }
            }
            ...
            if (maxScore > confidenceThreshold) {
                ...
                if (width > minBoxSize && height > minBoxSize) {
                    ...
                    Log.d(TAG, "Box too small: width=$width, height=$height")
                }
            }
        }
        return nms(results)
